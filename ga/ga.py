...
def get_user_source(analytics):
    ...
    for report in sources.get('reports', []):
        for row in report.get('data', {}).get('rows', []):
            sources_dict.append({'source': row.get('dimensions', [])[
                                0], 'amount': row.get('metrics', [])[0].get('values')[0]})
            ...
            'reportRequests': [
                {
                    ...
                    'metrics': [{'expression': 'ga:avgsearchDuration'},  # Average Search Duration
                                # Average amount of times a user clicked on a result
                                {'expression': 'ga:avgSearchResultViews'},
                                # How much subsequent pages were viewed after the search
                                {'expression': 'ga:avgSearchDepth'},
                                # Percentage of sessions with searches
                                {'expression': 'ga:percentSessionsWithSearch'},
                                # Did they find what they wanted on their first search
